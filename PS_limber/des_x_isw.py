...
def setup(options):
    ...
    dndz_filename = options.get_string(
        option_section, "matter_power", default="/Users/alessandromanzotti/Work/Software/cosmosis_new/cosmosis-standard-library/structure/PS_limber/data_input/DES/N_z_wavg_spread_model_0.2_1.2_tpz.txt")
...
def execute(block, config):
    ...
    cliswisw = [
        cl_limber_z(chispline, hspline, rbs, l, k1=ikern, k2=ikern, zmin=dndz[0, 0], zmax=dndz[-1, 0]) for l in lbins]

    cliswg = [cl_limber_z(chispline, hspline, rbs, l, k1=ikern, k2=gkern, zmin=dndz[0, 0], zmax=dndz[-1, 0])
              for l in lbins]

    clgg = [cl_limber_z(chispline, hspline, rbs, l, k1=gkern, k2=gkern, zmin=dndz[0, 0], zmax=dndz[-1, 0])
            for l in lbins]
    # =======================
    # SAVE IN DATABLOCK
