# this module contains classes and subroutines for working with temperature and polarization maps, as well as their fourier transforms.
# overview of classes:
#     * pix       - square map pixelization.
...
#     def Map_readFromAstroFITS(fname, hdu=0):
#         import pyfits
#         tf = pyfits.open(fname)[hdu]

#         shape  = ( tf.header['NAXIS2'], tf.header['NAXIS1'] )
#         center = ( tf.header['CRVAL1'], tf.header['CRVAL2'] )
#         reso   = tf.header['CD2_2'] # degrees
#         data   = np.nan_to_num(tf.data[::-1,::-1])

#         assert( tf.header['CTYPE1'] == 'RA---ZEA' )
#         assert( tf.header['CTYPE2'] == 'DEC--ZEA' )
#         assert( reso == np.abs(reso) )
#         assert( tf.header['CD2_1'] == 0 ); assert( tf.header['CD1_2'] == 0 )
#         assert( tf.header['CD1_1'] == -tf.header['CD2_2'] )
#         assert( data.shape == shape )
#         return sps.observation.sky.Map( name=fname, shape=shape, center=center, reso_arcmin=reso*60., map=data, projection=5 )

#     sps.observation.sky.Map.readFromAstroFITS = Map_readFromAstroFITS

#     def Map_get_ras_dcs(obj):
#         'returns (ras, dcs) with ras *not* wrapped 0-360'
#         ys, xs = np.meshgrid( np.arange(0,obj.shape[1]), np.arange(0,obj.shape[0]) )
#         ras, dcs = obj.pix2Ang( np.vstack( [xs.flatten(), ys.flatten()] ), wrap=False )
#         return ras.reshape(obj.shape), dcs.reshape(obj.shape)
#     sps.observation.sky.Map.get_ras_dcs = Map_get_ras_dcs

#     def Map_get_tht_phi(obj):
#         'returns (tht, phi) in radians with phi *not* wrapped 0-2pi'
#         ras, dcs = obj.get_ras_dcs()
#         (tht, phi) = math.radec_to_thetaphi(ras, dcs)
#         return tht, phi
#     sps.observation.sky.Map.get_tht_phi = Map_get_tht_phi

#     def Map_rebin(obj, tmpl, mask=None, intensive=True):
#         ras, dcs = obj.get_ras_dcs() #(xs, ys) = smap_lores.ang2Pix( [ras, dcs], round=False, return_validity=False )
#         (px, py), (vx, vy) = tmpl.ang2Pix( [ras.flatten(), dcs.flatten()], round=True, return_validity=True )

#         v = vx*vy
#         p = np.arange(0, np.prod(tmpl.shape)).reshape(tmpl.shape)
#         p = p[px[np.where(v==True)], py[np.where(v==True)]]

#         if mask == None:
#             mask = np.ones( ras.shape )

#         d = np.bincount( p, weights=(mask.flatten() * obj.map.flatten())[np.where(v == True)], minlength=np.prod(tmpl.shape) )
#         if intensive == True:
#             n = np.bincount( p, weights=mask.flatten()[np.where(v == True)], minlength=np.prod(tmpl.shape) )
#             d[ np.where( n != 0) ] /= n[ np.where( n != 0) ]

#         return sps.observation.sky.Map( name=obj.name, shape=tmpl.shape, center=tmpl.center,
#                                         reso_arcmin=tmpl.reso_arcmin, map=d.reshape(tmpl.shape), projection=tmpl.projection )

#     def Map_resample_healpix(obj, tmpl, mask=None, intensive=True, coord='G'):
#         import healpy as hp

#         nside = hp.npix2nside(len(obj))

#         ras, dcs = tmpl.get_ras_dcs()

#         rot = hp.rotator.Rotator(coord=['C',coord])
#         lns, lts = rot( ras.flatten(), dcs.flatten(), lonlat=True )
#         pxs = hp.vec2pix( nside, *hp.rotator.dir2vec(lns.flatten(), lts.flatten(), lonlat=True) )

#         if mask == None:
#             mask = np.ones( len(obj) )

#         d = np.bincount( np.arange(0, np.prod(tmpl.shape)), weights=(mask * obj)[pxs], minlength=np.prod(tmpl.shape) )
#         if intensive == True:
#             n = np.bincount( np.arange(0, np.prod(tmpl.shape)), weights=mask[pxs], minlength=np.prod(tmpl.shape) )
#             d[ np.where( n != 0) ] /= n[ np.where( n != 0) ]

#         return sps.observation.sky.Map( shape=tmpl.shape, center=tmpl.center,
#                                         reso_arcmin=tmpl.reso_arcmin, map=d.reshape(tmpl.shape), projection=tmpl.projection )

#     def Map_resample(obj, other, kx=1, ky=1, s=0):
#         import scipy.interpolate

#         ras, dcs = other.get_ras_dcs()
#         (xs, ys), (vx, vy) = obj.ang2Pix( [ras.flatten(), dcs.flatten()], round=False, return_validity=True )
#         sp_k     = scipy.interpolate.RectBivariateSpline(np.arange(0, obj.shape[0]), np.arange(0, obj.shape[1]), obj.map, kx=kx, ky=ky, s=s)
# 	return sps.observation.sky.Map( name=obj.name, shape=other.shape, center=other.center,
#                                         reso_arcmin=other.reso_arcmin, map=(sp_k.ev( xs.flatten(), ys.flatten() )*(vx*vy)).reshape(other.shape), projection=other.projection )

#     def Map_get_r(obj):
#         return rmap( obj.shape[1], obj.reso_rad, ny=obj.shape[0], map=obj.map )
#     sps.observation.sky.Map.get_r = Map_get_r

#     def PolarizedMap_get_tqu(obj):
#         return tqumap( obj.shape[1], obj.reso_rad, ny=obj.shape[0], maps=(obj['I'].map, obj['Q'].map, obj['U'].map) )
#     sps.observation.sky.PolarizedMap.get_tqu = PolarizedMap_get_tqu

#     def PolarizedMap_get_tqu_wt(obj):
#         return tqumap_wt( obj.shape[1], obj.reso_rad, ny=obj.shape[0], weight=obj.weight )
#     sps.observation.sky.PolarizedMap.get_tqu_wt = PolarizedMap_get_tqu_wt

# except:
#     print "failed to load sptpol_software"
...
class pix(object):
    ...
    def compatible(self, other):
        return ( (self.nx == other.nx) and
                 ...
                 np.allclose(self.dx, other.dx, rtol=1.e-8, atol=1.e-8) and
                 np.allclose(self.dy, other.dy, rtol=1.e-8, atol=1.e-8) )
...
class cfft(pix):
    ...
    def get_ml( self, lbins, w=None, psimin=0., psimax=np.inf, psispin=1 ):
        ...
        """
        assert(0); #FIXME: this function slated for removal, in favour of lcl.get_ml().
        ...
        norm, bins = np.histogram(ell, bins=lbins, weights=1.)
        clrr, bins = np.histogram(ell, bins=lbins, weights=w*c)

        clrr[ np.where(norm != 0.0) ] /= norm[ np.where(norm != 0.0) ]
        clrr[ np.where(wb != 0.0) ] /= wb
